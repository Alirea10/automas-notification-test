...
class Plugin:
    ...
    async def on_start(self) -> None:
        ...
        if not config.enabled:
            self.ctx.logger.info("[notification_test] 测试信息发送已关闭")
            return

        self._task = asyncio.create_task(self._send_after_channels_ready(config.delay_seconds))
        self.ctx.logger.info(
            f"[notification_test] 已计划在 {config.delay_seconds} 秒后发送通知测试信息"
        )

    async def on_stop(self, reason: str) -> None:
        if self._task is not None and not self._task.done():
            self._task.cancel()
        self.ctx.logger.info(f"[notification_test] 插件停止, reason={reason}")
    ...
    async def _send_after_channels_ready(self, delay_seconds: int) -> None:
        ...
        if notify is None:
            self.ctx.logger.warning("[notification_test] notify 服务不可用，跳过测试信息")
            return

        channels = self._channels(notify)
        if not channels:
            self.ctx.logger.warning("[notification_test] 当前没有已注册通知渠道，测试信息未发送")
            return
        ...
        self.ctx.logger.info(
            f"[notification_test] 测试信息发送完成，成功={succeeded or '无'}，失败={failed or '无'}"
        )
